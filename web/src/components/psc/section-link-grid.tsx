  ...
  useOnWindow('load', $(async () => {
    try {
      if (!Array.isArray(props.sections)) {
        console.error('Expected props.sections to be an array, but got:', typeof props.sections);
        completions.value = [];
        done.value = [];
        return;
      }
      ...
      completions.value = await Promise.all(
        props.sections.map(async (section) => {
          try {
            return await getPercentCompletion(section);
          } catch (error) {
            console.error('Error in getPercentCompletion for section:', section, error);
      ...
      // Count of completed items, per section
        ...
        props.sections.map((section) => {
          if (Array.isArray(section.checklist)) {
            const filteredItems = section.checklist.filter((item) => {
              const key = item.point.toLowerCase().replace(/ /g, '-');
              return checked.value && checked.value[key];
            });
            return filteredItems.length;
          } else {
            console.error('Expected section.checklist to be an array, but got:', typeof section.checklist);
            return 0; // Default value in case section.checklist is not an array
          }
        })
      );
  
    } catch (error) {
      console.error('Error during load event handling:', error);
      completions.value = [];
      done.value = [];
    }
  }));
